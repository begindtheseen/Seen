...
// ── Shared helpers for the keyless secondary sources ────────────────────────
// Strip HTML, collapse whitespace, cap length — same transform Adzuna uses.
...
// Build a job object in the exact shape upsertJobs() expects, applying the same
// junk filter Adzuna applies. Returns null if the job fails quality standards.
function buildJob({ title, company, location, salary, description, apply_url, source, type, searchQuery, salaryMin = 0 }) {
...
// Generic fetch-with-timeout wrapper (8s, like fetchAdzuna). Returns parsed JSON or null.
...
// Pulls a few pages to add volume; each page returns ~20 jobs.
