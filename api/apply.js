...
<tr><td style="background:#111114;border:1px solid #1e1e28;border-radius:16px;padding:40px 36px;">
  ...
  <div style="background:#00ff8712;border:1px solid #00ff8728;border-radius:8px;padding:14px 18px;font-size:12px;color:#00ff87;line-height:1.65;">
    → If you don't hear back within 30 days, come back and report it. Every report helps the next person.
  </div>
</td></tr>
<tr><td align="center" style="padding-top:24px;font-size:11px;color:#2a2a3a;line-height:1.8;">
  You applied through Seen · <a href="https://seenjobs.io" style="color:#2a2a3a;">seenjobs.io</a>
</td></tr>
</table></td></tr></table>
</body></html>`
      })
    ...
    emailsToSend.push(fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `New application via Seen — ${role} at ${company}`,
...
<tr><td>
  <div style="background:#fff;border:1px solid #e8e8f0;border-radius:12px;padding:32px;">
    ...
    <p style="margin:0 0 24px;font-size:14px;color:#6a6a88;">Applied for <strong>${role}</strong> at ${company} · ${location}</p>
    ...
    <a href="mailto:${applicantEmail}?subject=Re: Your application for ${role}" style="display:inline-block;padding:12px 24px;background:#4f46e5;color:#fff;text-decoration:none;border-radius:8px;font-size:14px;font-weight:700;">Reply to applicant →</a>
  </div>
  <p style="text-align:center;font-size:11px;color:#9898b0;margin-top:16px;">Sent via Seen · seenjobs.io · The transparent job board</p>
</td></tr>
</table>
</body></html>`
      })
