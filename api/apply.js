...
export default async function handler(req) {
  ...
  try {
    ...
    // Build attachment if we have readable resume text
    const resumeAttachment = resumeText ? [{
      filename: `${(applicantName || 'Resume').replace(/\s+/g, '_')}_Resume.txt`,
      content: toBase64(resumeText),
    }] : undefined;
...
<tr><td style="background:#111114;border:1px solid #1e1e28;border-radius:16px;padding:40px 36px;">
  ...
  <div style="background:#1c1c26;border:1px solid #2e2e3e;border-radius:10px;padding:16px 20px;margin-bottom:24px;">
    ...
    <div style="font-size:14px;color:#e4e4f0"><strong style="color:#9898b0">Resume:</strong> ${resumeText ? `<span style="color:#00ff87">✓ Attached${resumeLink === 'AI-optimized resume (Seen)' ? ' · AI-optimized for this role' : ''}</span>` : `<span style="color:#e4e4f0">${resumeLink || 'On file'}</span>`}</div>
...
<tr><td>
  <div style="background:#fff;border:1px solid #e8e8f0;border-radius:12px;padding:32px;">
    ...
    <div style="background:#f4f4f8;border-radius:8px;padding:16px;margin-bottom:20px">
      ...
      <div style="font-size:14px;color:#0f0f18;margin-bottom:6px"><strong>Resume:</strong> ${resumeText ? `<span style="color:#15803d">✓ Attached as file${resumeLink === 'AI-optimized resume (Seen)' ? ' (AI-optimized)' : ''}</span>` : (resumeLink?.startsWith('http') ? `<a href="${resumeLink}" style="color:#4f46e5">${resumeLink}</a>` : `<span>${resumeLink || 'On file'}</span>`)}</div>
...
${resumeText ? `<p>Resume: attached</p>` : `<p>Resume: ${resumeLink?.startsWith('http') ? `<a href="${resumeLink}">${resumeLink}</a>` : (resumeLink || 'On file')}</p>`}
