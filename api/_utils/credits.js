...
export async function gateAI(req, reason = 'ai_tool') {
  ...
  if (JWT_SECRET) {
    ...
    if (!payload) return { ok: false, status: 401, error: 'Session expired — sign in again', credits_required: true };
    uid = payload.sub;
  } else {
  ...
  if (!cred) {
    await db('ai_credits', { method: 'POST', headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ user_id: uid, balance: 2, daily_earned: 0, last_reset: today, pro: false }) });
    await db('credit_transactions', { method: 'POST', headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ user_id: uid, delta: -1, reason }) });
    return { ok: true, uid, balance: 2, pro: false };
  }
  ...
  if (cred.balance <= 0) {
    return { ok: false, status: 402, error: 'No AI credits remaining — earn more or go Pro', credits_required: true, balance: 0 };
  }
