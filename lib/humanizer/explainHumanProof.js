...
import { HUMANPROOF_WEIGHTS, } from './types.js';
const REASONS = {
...
export function computeHumanProofScore(b) {
    let score = 0;
    for (const k of Object.keys(HUMANPROOF_WEIGHTS)) {
...
export function aiSlopRiskFromScore(score) {
    if (score >= 70)
        return 'low';
    if (score >= 45)
        return 'medium';
...
export function explainHumanProof(b) {
    const contributions = Object.keys(HUMANPROOF_WEIGHTS).map((k) => {
    ...
    const summary = `HumanProof Score ${humanproof_score}/100 (${ai_slop_risk} AI-slop risk). ` +
        `Strongest: ${labelOf(topDriver.factor)} (${topDriver.points} pts). ` +
        `Biggest opportunity: ${labelOf(biggestGap.factor)}.`;
    const notes = [];
    if (b.buzzword_control < 0.6)
        notes.push('Several generic/AI-style phrases remain — replacing them with specifics will raise the score.');
    if (b.evidence_grounding < 0.6)
        notes.push('Some claims aren\'t backed by your résumé — confirm or remove them before using.');
    if (b.interview_defensibility < 0.6)
        notes.push('A few lines would be hard to defend in an interview — make them concrete.');
    if (b.specificity < 0.5)
        notes.push('Add real work context (what you did, with what, for whom) to lift specificity.');
