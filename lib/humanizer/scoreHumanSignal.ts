...
import type {
  HumanSignalBreakdown, ResumeFact, JobFact, RoleFamily, PhraseRiskRule,
} from './types.ts';
...
function voiceConsistency(text: string): number {
  const bullets = splitBullets(text);
  if (bullets.length < 2) return 0.9;
...
function roleBelievability(text: string, ctx: ScoreContext): number {
  ...
  let s = 0.85;
  if (ctx.roleFamily !== 'general') s += 0.1; // a recognized family → grounded context available
  if (seniorityRisk) s -= 0.3;
  if (overpolished) s -= 0.2;
  return clamp01(s);
}

function formattingNaturalness(text: string): number {
  const bullets = splitBullets(text);
  if (bullets.length < 2) return 0.85;
  ...
  let s = 0.9;
  if (allCaps > 0) s -= 0.3;
  if (uniformEnd && bullets.length > 4) s -= 0.05;
  return clamp01(s);
}

export function scoreHumanSignal(text: string, ctx: ScoreContext): HumanSignalBreakdown {
