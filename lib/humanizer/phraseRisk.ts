...
import type { PhraseRiskRule, FlagType, FlagSeverity } from './types.ts';
...
export const BUILTIN_PHRASE_RULES: PhraseRiskRule[] = [
...
function rule(phrase: string, category: FlagType, severity: FlagSeverity, replacementHint: string | null): PhraseRiskRule {
...
export function mergeRules(extra?: PhraseRiskRule[] | null): PhraseRiskRule[] {
  if (!extra || !extra.length) return [...BUILTIN_PHRASE_RULES];
  const byPhrase = new Map<string, PhraseRiskRule>();
  for (const r of BUILTIN_PHRASE_RULES) byPhrase.set(r.phrase, r);
  for (const r of extra) {
    if (r && r.phrase) byPhrase.set(String(r.phrase).toLowerCase(), { ...r, phrase: String(r.phrase).toLowerCase() });
  }
  return [...byPhrase.values()];
}

// Case-insensitive, whole-phrase, word-boundary-ish matcher. Returns every match span
// (index + the exact matched text, preserving original casing) for a phrase.
export interface PhraseMatch {
  rule: PhraseRiskRule;
  index: number;
  matched: string; // original-cased surface span from the text
}

export function findPhraseMatches(text: string, rules: PhraseRiskRule[]): PhraseMatch[] {
