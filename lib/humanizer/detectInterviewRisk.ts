...
import type { InterviewRiskWarning, ResumeFact, HumanProofFlag } from './types.ts';
...
const RISKY_PATTERNS: Array<{ re: RegExp; warning: string }> = [
...
export interface InterviewRiskResult {
  warnings: InterviewRiskWarning[];
  flags: HumanProofFlag[];
  defensibility: number; // 0–1, higher = more defensible / fewer risky claims
}

export function detectInterviewRisk(text: string, resumeFacts: ResumeFact[]): InterviewRiskResult {
  const src = String(text || '');
  const warnings: InterviewRiskWarning[] = [];
