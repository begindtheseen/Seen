...
import type { ResumeFact, JobFact, RoleFamily, PhraseRiskRule } from './types.ts';
...
export const ROLE_DETAILS: Record<RoleFamily, {
  fastPaced: string;        // replaces "fast-paced environment"
  communication: string[];  // replaces "strong communication skills" (pick first supported)
  duties: string[];         // candidate concrete duties for "various duties"
  contexts: string[];       // generic believable contexts for this family
}> = {
  restaurant_service: {
...
const FAMILY_SIGNALS: Array<{ family: RoleFamily; words: RegExp }> = [
...
export function inferRoleFamily(opts: {
  ...
  if (opts.roleFamily && opts.roleFamily !== 'general') return opts.roleFamily;
  const haystack = [
    opts.jobTitle || '',
    opts.text || '',
  ...
  for (const sig of FAMILY_SIGNALS) {
    if (sig.words.test(haystack)) return sig.family;
  }
  return 'general';
}
...
function dutiesFromFacts(family: RoleFamily, resumeFacts: ResumeFact[]): string[] {
  ...
  const supported = detail.duties.filter((d) =>
    factWords.some((w) => d.toLowerCase().includes(w) || w.includes(d.toLowerCase().split(' ')[0]))
  );
...
export function resolveContextReplacement(
  rule: PhraseRiskRule,
  ctx: { roleFamily: RoleFamily; resumeFacts: ResumeFact[]; jobFacts?: JobFact[] }
): string | null {
  const detail = ROLE_DETAILS[ctx.roleFamily];
  const phrase = rule.phrase;

  if (/fast-?paced environment/.test(phrase)) return detail.fastPaced;
