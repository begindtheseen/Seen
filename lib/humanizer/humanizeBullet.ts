...
import { findPhraseMatches, matchCase } from './phraseRisk.ts';
import { resolveContextReplacement, inferRoleFamily, ROLE_DETAILS } from './roleDetails.ts';

export interface HumanizeBulletResult {
  text: string;
  changed: boolean;
  evidenceUsed: string[];
...
export function humanizeBullet(
  bullet: string,
  opts: {
  ...
  if (!original.trim()) return { text: original, changed: false, evidenceUsed: [], changes: [] };
...
export function stripUnsupportedClaims(
  text: string,
  unsupported: { metrics: string[]; tools: string[] }
): { text: string; changes: Array<{ from: string; to: string; reason: string }> } {
  let out = String(text || '');
  const changes: Array<{ from: string; to: string; reason: string }> = [];
