...
import { discoveryPageFor } from '../../api/refresh-jobs.js';
import { pageFor } from '../../api/discover-sources.js';
...
function crawlFetch({ collinfo = [{ id: 'CC-MAIN-2026-33' }], rows = {}, delayMs = 0, failCollinfo = false } = {}) {
  const calls = [];
...
test('discoverFromCommonCrawl puts the requested page in every CDX url (rotation is honoured)', async () => {
  const fetchImpl = crawlFetch();
  await discoverFromCommonCrawl({ fetchImpl, page: 7, patterns: ['boards.greenhouse.io/*', 'jobs.lever.co/*'] });
  const cdx = fetchImpl.calls.filter((u) => u.includes('-index?'));
  assert.equal(cdx.length, 2);
  for (const u of cdx) assert.ok(u.includes('&page=7'), `page not propagated: ${u}`);
});
...
test('discoveryPageFor advances once per 4h cron run and wraps', () => {
  const base = 1_000_000 * 14_400_000; // an exact 4h boundary
  const p0 = discoveryPageFor(base);
  const p1 = discoveryPageFor(base + 14_400_000);
  assert.notEqual(p0, p1, 'consecutive runs must not rescan the same page');
  assert.equal(p1, (p0 + 1) % 40);
  assert.equal(discoveryPageFor(base + 40 * 14_400_000), p0, 'wraps after DISCOVERY_PAGES');
  // Two calls inside the same run window agree.
  assert.equal(discoveryPageFor(base + 60_000), p0);
});
