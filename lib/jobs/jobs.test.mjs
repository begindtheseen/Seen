...
import { pageFor, summarizeAdzunaProbe, sweepOptionsFromParams } from '../../api/discover-sources.js';
...
// ── Lever EU-region host fallback ────────────────────────────────────────────
// Lever's public Postings API is split by data-residency host: a board hosted on jobs.eu.lever.co
// 404s (empty response) against the global api.lever.co and must be read from api.eu.lever.co
// instead. Measured live in production 2026-09-26: 53 of 54 registered EU-hosted Lever boards had
// never once synced despite being confirmed as picked up and queried by the scheduler (#295) — only
// calling the wrong host explains a failure rate that high against real, presumably-hiring companies.
test('Lever falls back to the EU host when the global host returns nothing', async () => {
  const calls = [];
  const fetchImpl = async (url) => {
    calls.push(url);
    if (url.startsWith('https://api.eu.lever.co/')) {
      return { ok: true, json: async () => [
        { id: 'eu-1', text: 'Backend Engineer', categories: { location: 'Stockholm', commitment: 'Full-time' }, hostedUrl: 'https://jobs.eu.lever.co/seb/eu-1', descriptionPlain: DESC },
      ] };
    }
    // Global host: a 404-shaped "not found" — mirrors what getJson collapses a real 404 into.
    return { ok: false, status: 404, json: async () => ({}) };
  };
  const rows = await lever.fetchCompanyJobs('seb', { companyName: 'SEB', fetchImpl });
  assert.equal(rows.length, 1, 'EU-hosted tenant must still resolve via the fallback host');
  assert.equal(rows[0].ats_tenant, 'seb');
  assert.equal(rows[0].apply_url, 'https://jobs.eu.lever.co/seb/eu-1');
  assert.deepEqual(calls, [
    'https://api.lever.co/v0/postings/seb?mode=json',
    'https://api.eu.lever.co/v0/postings/seb?mode=json',
  ], 'must try the global host first, EU host only as a fallback');
});

test('Lever does not fall back when the global host already has data (no wasted request)', async () => {
  const calls = [];
  const fetchImpl = async (url) => {
    calls.push(url);
    return { ok: true, json: async () => [{ id: 'g-1', text: 'Engineer', hostedUrl: 'https://jobs.lever.co/netflix/g-1', descriptionPlain: DESC }] };
  };
  const rows = await lever.fetchCompanyJobs('netflix', { fetchImpl });
  assert.equal(rows.length, 1);
  assert.equal(calls.length, 1, 'a successful global-host response must not trigger the EU fallback');
});

test('Lever returns empty (never throws) when both hosts genuinely have nothing', async () => {
  const rows = await lever.fetchCompanyJobs('dead-tenant', { fetchImpl: mockFetch(null, { ok: false }) });
...
// ── The manual-sweep route: caller-controlled CONCURRENCY, clamped ─────────────
// The discover-sources handler exposes ?concurrency so a manual sweep can be dialled up or down, but
// data.commoncrawl.org is a free public service — the clamp is what keeps a hand-crafted request from
// hammering it. Pinned here so the caps cannot quietly widen. discoverFromCommonCrawl itself is what
// then runs the sweep concurrently (see the concurrency-cap test above); this covers the wiring.
test('discover-sources sweepOptionsFromParams clamps concurrency and defaults sensibly', () => {
  const at = 900_000 * (12 * 60 * 60 * 1000); // a fixed instant so page is deterministic
  const opts = (qs) => sweepOptionsFromParams(new URLSearchParams(qs), at);

  // Default when unset: matches the library default, sweep runs concurrently, not serially.
  assert.equal(opts('').concurrency, 4, 'absent ?concurrency must default to the concurrent library default');

  // A caller may dial DOWN to fully serial…
  assert.equal(opts('concurrency=1').concurrency, 1, 'concurrency=1 (serial) is allowed');
  // …and UP, but never past the hard cap, no matter how large the ask.
  assert.equal(opts('concurrency=8').concurrency, 8, 'concurrency at the cap is honoured');
  assert.equal(opts('concurrency=999').concurrency, 8, 'a huge ?concurrency is clamped to the cap');
  // Hostile / degenerate input floors at 1 rather than 0 or negative (which mapLimit could not run).
  assert.equal(opts('concurrency=0').concurrency, 1, '0 floors to 1');
  assert.equal(opts('concurrency=-5').concurrency, 1, 'negative floors to 1');
  assert.equal(opts('concurrency=abc').concurrency, 4, 'non-numeric falls back to the default');

  // page and blocks are parsed and clamped alongside concurrency, and page falls back to the cron slot.
  assert.equal(opts('page=3&blocks=2').page, 3);
  assert.equal(opts('blocks=99').maxBlocksPerPattern, 24, 'blocks clamped to 24');
  assert.equal(opts('blocks=0').maxBlocksPerPattern, 1, 'blocks floors to 1');
  assert.equal(opts('').page, pageFor(at), 'absent ?page falls back to the rotating cron slot');
});
...
function registryFetch(rows) {
  ...
  return Object.assign(async (url) => {
    ...
    const providerMatch = /[?&]provider=eq\.([^&]+)/.exec(url);
    const provider = providerMatch ? decodeURIComponent(providerMatch[1]) : null;
    let pool = rows.filter(r => (r.last_successful_sync == null) === wantNull);
    if (provider) pool = pool.filter(r => r.provider === provider);
...
const board = (id, sync, fails = 0, provider = 'greenhouse') => ({ id, company_name: id, provider, tenant: id, consecutive_failures: fails, last_successful_sync: sync });
...
// ── dueSources: a dominant provider's own backlog must not starve a minority provider ──────────
// Reproduces the live shape found in production on 2026-09-25: Greenhouse alone had ~4,000
// registered tenants against ~150 Ashby and ~55 Lever, and Greenhouse's re-sync backlog contained
// over 2,000 tenants OLDER than Ashby's or Lever's own most-overdue row. A plain
// `order=last_successful_sync.asc` limited to a handful of proven slots never reached Ashby or Lever
// at all — both went silently unsynced for 10-15+ days despite each being individually far more
// overdue than most Greenhouse rows.
test('dueSources round-robins the proven pool across providers so a dominant one cannot starve a minority', async () => {
  const greenhouseBacklog = Array.from({ length: 2000 }, (_, i) =>
    board(`gh-${i}`, `2026-08-0${(i % 9) + 1}T00:00:00Z`));
  const ashby = board('ashby-1', '2026-09-15T00:00:00Z', 0, 'ashby');
  const lever = board('lever-1', '2026-09-10T00:00:00Z', 0, 'lever');
  const out = await dueSources('https://db.test', 'k', 24, {
    fetchImpl: registryFetch([...greenhouseBacklog, ashby, lever]),
  });
  assert.ok(out.some(r => r.provider === 'ashby'), 'ashby starved despite being long overdue');
  assert.ok(out.some(r => r.provider === 'lever'), 'lever starved despite being long overdue');
  assert.ok(out.some(r => r.provider === 'greenhouse'), 'greenhouse must still get slots too');
});
