...
// DEGRADE_AT existing.
//
// PER-PROVIDER FAIRNESS. A single global staleness ordering WITHIN a pool has the exact same
// crowding failure mode described above, just one level down: it starves a minority ATS provider the
// moment a dominant one's own backlog outgrows what a run's slots can drain. Measured live in
// production 2026-09-25: Greenhouse alone had grown to ~4,000 registered tenants against ~150 Ashby
// and ~55 Lever, and Greenhouse's own re-sync backlog contained over 2,000 tenants OLDER than
// Ashby's or Lever's most-overdue row — so a plain `order=last_successful_sync.asc` never reached
// Ashby or Lever at all, even though each was individually far more overdue than most Greenhouse
// rows. Ashby went fully unsynced for 10+ days and Lever for 15+, both silently: recordSourceSync
// never ran for them, so consecutive_failures stayed low and nothing looked broken. Querying each
// known provider's own most-overdue slice SEPARATELY, then round-robining one-per-provider, caps how
// much of a pool any single provider can claim — a large registry can shrink a small one's SHARE of a
// run but can never again make it wait indefinitely for a turn.
function roundRobinByProvider(perProviderLists) {
  const queues = perProviderLists.filter((q) => q.length);
  const out = [];
  let i = 0;
...
export async function dueSources(supabaseUrl, serviceKey, limit = 40, { provenShare = 1 / 3, fetchImpl = fetch } = {}) {
  ...
  // One query per known ingestable provider (a fixed, small set), so each provider's staleness
  // ordering is computed against only ITS OWN rows and can never be buried behind another
  // provider's backlog before the round-robin below even sees it.
  const providers = [...INGESTABLE_PROVIDERS];
  const pullPoolByProvider = async (filter, order, n) => {
    const perProvider = await Promise.all(
      providers.map((p) => pull(`${filter}&provider=eq.${encodeURIComponent(p)}`, order, n))
    );
    return roundRobinByProvider(perProvider);
  };
  ...
  const [fresh, proven] = await Promise.all([
    pullPoolByProvider('last_successful_sync=is.null', 'consecutive_failures.asc,first_discovered_at.asc', freshSlots + provenSlots),
    pullPoolByProvider('last_successful_sync=not.is.null', 'last_successful_sync.asc', provenSlots + freshSlots),
  ]);
