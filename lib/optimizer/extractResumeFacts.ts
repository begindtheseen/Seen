...
import type { ResumeFact, FactType } from './types.ts';
...
function uniqueByLabel(facts: ResumeFact[]): ResumeFact[] {
  const seen = new Set<string>();
  const out: ResumeFact[] = [];
...
export function extractResumeFacts(
  ...
  for (const phrase of phrases) {
    ...
    const factType: FactType = isTool(normalized) ? 'tool' : 'skill';
    facts.push({
      factType,
      label: phrase,
...
function isTool(normalized: string): boolean {
...
export function resumeSkillLabels(facts: ResumeFact[]): string[] {
...
export function resumeSeniorityRank(facts: ResumeFact[]): number {
