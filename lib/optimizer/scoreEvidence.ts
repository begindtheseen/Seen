...
import type { ResumeFact, RequirementMatch } from './types.ts';

export interface EvidenceResult {
  evidence_strength: number; // 0–1
  metric_count: number;
  matched_with_evidence: number;
  confidence: number; // 0–1 overall run confidence
}
...
export function scoreEvidence(
  ...
  const evidence_strength = clamp01(
    evidenceCoverage * 0.55 + metricBonus * 0.3 + titleBonus * 0.15
  );
