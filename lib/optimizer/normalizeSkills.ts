...
export interface SkillTaxonomy {
  // alias (lower) -> canonical skill (lower)
  aliases: Record<string, string>;
  // canonical skill -> related canonical skills with a 0–1 weight
  relationships: Record<string, Array<{ to: string; weight: number }>>;
}
...
const RELATIONSHIPS: Record<string, Array<{ to: string; weight: number }>> = {
  'customer service': [
...
export const BUILTIN_TAXONOMY: SkillTaxonomy = {
  aliases: ALIASES,
...
export function mergeTaxonomy(extra?: Partial<SkillTaxonomy> | null): SkillTaxonomy {
  if (!extra) return BUILTIN_TAXONOMY;
  return {
...
export function normalizeSkill(raw: string, taxonomy: SkillTaxonomy = BUILTIN_TAXONOMY): string {
  const c = canonicalText(raw);
  if (!c) return '';
  // direct alias hit
  if (taxonomy.aliases[c]) return taxonomy.aliases[c];
  // trailing plurals / common suffixes
  const singular = c.replace(/(skills?|abilities|experience)$/i, '').trim();
  if (singular && taxonomy.aliases[singular]) return taxonomy.aliases[singular];
  return singular || c;
}

export type RelationKind = 'exact' | 'related' | 'none';
...
export function bestMatch(
  ...
  resumeSkills: string[],
  taxonomy: SkillTaxonomy = BUILTIN_TAXONOMY
): { kind: RelationKind; strength: number; via: string | null } {
  let best: { kind: RelationKind; strength: number; via: string | null } = {
    kind: 'none',
    strength: 0,
    via: null,
  };
  for (const rs of resumeSkills) {
    ...
    if (r.strength > best.strength) best = { kind: r.kind, strength: r.strength, via: rs };
  }
  return best;
}
