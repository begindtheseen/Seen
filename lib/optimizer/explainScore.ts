...
import { SEENFIT_WEIGHTS, type ScoreBreakdown, type ScoreContribution } from './types.ts';

const REASONS: Record<keyof typeof SEENFIT_WEIGHTS, (raw: number, b: ScoreBreakdown) => string> = {
  required_match: (r) =>
    `You match ${pct(r)} of the role's required skills (weighted by importance).`,
  preferred_match: (r) =>
    `You match ${pct(r)} of the role's preferred / nice-to-have skills.`,
  evidence_strength: (r) =>
    `Your résumé backs its claims at ${pct(r)} strength — matched skills appearing inside real accomplishment lines and metrics score higher.`,
  seniority_alignment: (r) =>
    `Your seniority aligns with the role at ${pct(r)} (gaps in level reduce this).`,
  company_process_score: (_r, b) =>
    `The company's hiring process scores ${Math.round(b.company_process_score)}/100 from real applicant outcomes (response/ghost rates).`,
  ats_coverage: (r) =>
    `${pct(r)} of the job's keywords literally appear in your résumé — important for ATS keyword filters.`,
  location_pay_alignment: (r) =>
    `Location / pay alignment is ${pct(r)}.`,
  risk_penalty: (r) =>
    r >= 0.99
...
function rawValues(b: ScoreBreakdown): Record<keyof typeof SEENFIT_WEIGHTS, number> {
  return {
...
export function computeFitScore(b: ScoreBreakdown): number {
  const raw = rawValues(b);
  let score = 0;
  for (const k of Object.keys(SEENFIT_WEIGHTS) as Array<keyof typeof SEENFIT_WEIGHTS>) {
    score += SEENFIT_WEIGHTS[k] * raw[k];
  }
  return Math.max(0, Math.min(100, Math.round(score * 100)));
}

export function explainScore(b: ScoreBreakdown): {
  fit_score: number;
  summary: string;
  ...
  const raw = rawValues(b);
  const contributions: ScoreContribution[] = (Object.keys(SEENFIT_WEIGHTS) as Array<keyof typeof SEENFIT_WEIGHTS>).map(
    (k) => {
  ...
  const summary =
    `SeenFit Score ${fit_score}/100. Biggest driver: ${labelOf(topDriver.factor)} (${topDriver.points} pts). ` +
    `Biggest opportunity: improve ${labelOf(biggestGap.factor)}.`;

  const notes: string[] = [];
  if (b.required_match < 0.5) notes.push('You are missing several required skills — see "Missing keywords".');
  if (b.ats_coverage < 0.5) notes.push('Many job keywords are absent from your résumé text — add the real ones you have.');
  if ((b.company_process_score ?? 50) < 40) notes.push('This company has a poor hiring-process track record (ghosting/no response).');
  if (b.risk_penalty < 0.99) notes.push('A risk penalty was applied — review the risk flags before relying on this score.');
