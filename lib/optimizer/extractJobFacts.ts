...
import type { JobFact, FactImportance } from './types.ts';
import { canonicalText, normalizeSkill, BUILTIN_TAXONOMY, type SkillTaxonomy } from './normalizeSkills.ts';
...
const SENIORITY_PATTERNS: Array<{ re: RegExp; level: string; rank: number }> = [
...
function findPhrase(haystack: string, needle: string): boolean {
...
export function extractJobFacts(
  ...
  opts: { jobTitle?: string; taxonomy?: SkillTaxonomy } = {}
): JobFact[] {
  ...
  const found = new Map<string, { importance: FactImportance; label: string }>();
  const rank: Record<FactImportance, number> = { nice_to_have: 0, preferred: 1, required: 2 };

  for (const clause of clauses) {
    const lc = canonicalText(clause);
    const isReq = REQUIRED_CUE.test(clause);
    const isPref = PREFERRED_CUE.test(clause);
    const importance: FactImportance = isPref && !isReq ? 'preferred' : isReq ? 'required' : 'nice_to_have';
    for (const phrase of phrases) {
      const needle = canonicalText(phrase);
      if (!findPhrase(lc, needle)) continue;
  ...
  for (const [normalized, { importance, label }] of found) {
    let imp = importance;
    if (!anyRequired && imp === 'nice_to_have') imp = 'required';
    facts.push({
      factType: 'skill',
      label,
...
export function jobSeniorityRank(facts: JobFact[]): number {
  const levelRank: Record<string, number> = { entry: 1, mid: 2, senior: 3, lead: 4, director: 5, executive: 6 };
  for (const f of facts) {
    if (f.factType === 'seniority') return levelRank[f.label] || 0;
  }
  return 0;
}
