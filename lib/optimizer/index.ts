...
import { mergeTaxonomy, type SkillTaxonomy } from './normalizeSkills.ts';
import { extractResumeFacts, resumeSeniorityRank } from './extractResumeFacts.ts';
import { extractJobFacts, jobSeniorityRank } from './extractJobFacts.ts';
import { scoreFit, seniorityAlignment } from './scoreFit.ts';
import { scoreAts } from './scoreAts.ts';
import { scoreEvidence, confidenceLabel } from './scoreEvidence.ts';
import { generateSafeBullets } from './generateSafeBullets.ts';
import { generateApplicationMessage } from './generateApplicationMessage.ts';
import { explainScore } from './explainScore.ts';
...
function locationPayAlignment(input: OptimizeInput): number {
  if (input.remoteOk) return 1;
  const want = (input.desiredLocation || '').toLowerCase().trim();
  const have = (input.jobLocation || '').toLowerCase().trim();
  if (!want || !have) return 0.7; // unknown → neutral-positive
  if (have.includes('remote') || want.includes('remote')) return 1;
  if (have.includes(want) || want.includes(have)) return 1;
...
export interface RunOptions {
  taxonomy?: Partial<SkillTaxonomy> | null;
}

export function runOptimizer(input: OptimizeInput, options: RunOptions = {}): OptimizerOutput {
  const taxonomy: SkillTaxonomy = mergeTaxonomy(options.taxonomy);

  // 1. Facts.
  const resumeFacts = extractResumeFacts(input.resumeText || '', taxonomy);
  const jobFacts: JobFact[] =
    input.jobFacts && input.jobFacts.length
      ? input.jobFacts
