...
import { canonSkill, skillRelation } from './skillGraph.js';
...
export function assessRequirement(req, resume) {
  ...
  // 1. Exact skill present.
  if (resumeSkills.includes(reqCanon)) {
    const node = (resume.evidenceNodes || []).find(n => n.type === 'skill' && canonSkill(n.label) === reqCanon);
    return finalize(req, 'exact', 0.95, node ? [node.evidenceText] : [reqCanon],
      `Your résumé directly shows "${reqCanon}".`);
  }
