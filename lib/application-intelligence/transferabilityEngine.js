...
export function assessTransferability(resume, job) {
  ...
  // Responsibilities are context (their label is a raw JD sentence), not a skill the résumé can
  // "have" or "miss" — never surface them as a gap. Only real skills/credentials/experience can
  // be a missing requirement.
  const isGap = (a) => a.matchType === 'missing' && a.importance !== 'responsibility';
  const missingRisky = all.filter(a => isGap(a) && (a.missSeverity === 'fatal' || a.missSeverity === 'risky'));
  const missingRecoverable = all.filter(a => isGap(a) && a.missSeverity === 'recoverable');
  return {
    assessments: all,
    exact: bucket(['exact']),
