...
// Catches money ("$1.2M"), percentages ("95%"), multipliers ("3.2x"), and — industry-agnostic
// — any quantity of a plural noun ("60+ guest vehicles", "120 students", "3 apprentices",
// "40+ job sites", "5 sections"). This last branch is what makes metric capture work for ANY
// field, not just the service units we happen to have listed.
const METRIC_RE = /(\$\s?\d[\d,]*(?:\.\d+)?\s*(?:k\b|m\b|million|billion|\/hr)?|\d[\d,]*(?:\.\d+)?\s*%|\d[\d,]*(?:\.\d+)?\s*x\b|\d[\d,]*\+?(?:\s+[a-z]+){0,2}\s+[a-z]{3,}s\b|\d[\d,]*\+?(?:\s+[a-z]+){0,2}\s+(?:customers?|clients?|guests?|vehicles?|tickets?|calls?|orders?|deliveries|stops?|packages?|patients?|users?|accounts?|reports?|invoices?|forms?|records?|employees?|people|transactions?|shifts?|routes?|items?|units?|hours?))/gi;
const BULLET_RE = /^\s*[•\-*▪◦·‣]\s+/;
...
function nodesForText(text, roleId, bulletIdx) {
  ...
  // skill/tool mentions via alias/canonical surfaces present in the text (word-boundary safe:
  // "register" must not match "registered", "pos" must not match "position")
  const surfaces = new Set();
  for (const surf of KNOWN_SURFACES) {
    if (hasWord(lower, surf)) surfaces.add(canonSkill(surf));
  }
  let si = 0;
...
export function parseResumeV2(resumeText) {
  ...
  // Body lines to segment into roles: the Experience section if the résumé labels one,
  // otherwise everything that wasn't a Summary/Skills/Education/Certs/Projects section — so
  // résumés with no explicit "Experience" header still parse their roles.
  const bodyLines = experienceLines.length ? experienceLines : sections.header;
  ...
  for (const surf of KNOWN_SURFACES) if (hasWord(skillsSectionText, surf)) skillSet.add(canonSkill(surf));
  // OPEN-VOCABULARY: every phrase listed in the Skills section is a candidate skill, so the
  // engine recognizes SEO, curriculum design, NEC code, CAD — anything, in any industry —
  // without needing a dictionary entry.
  const skillPhrases = sections.skills.join('\n')
    .split(/[\n,;|•·]|\s\/\s|\s+and\s+(?=[a-z])/i)
    .map(s => s.replace(/^[\s\-*:]+/, '').trim().toLowerCase())
    .filter(s => s.length >= 2 && s.length <= 40 && /[a-z]/.test(s)
      && !/^(skills?|technical|core|competenc\w*|proficienc\w*|technolog\w*|tools?|summary|experience)\b/.test(s));
  for (const p of skillPhrases) skillSet.add(canonSkill(p));
