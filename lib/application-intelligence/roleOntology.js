...
export const ROLE_FAMILIES = {
  ...
  marketing: {
    label: 'Marketing',
    titles: ['marketing', 'seo', 'content', 'social media', 'brand', 'growth', 'demand generation', 'communications', 'copywriter', 'digital marketing'],
    keywords: ['seo', 'sem', 'content marketing', 'social media', 'google analytics', 'paid media', 'email marketing', 'campaigns', 'brand', 'copywriting', 'hubspot', 'ppc'],
    provides: ['content marketing', 'communication', 'reporting', 'campaign management', 'analytics'],
    strongVerbs: ['launched', 'grew', 'created', 'optimized', 'managed', 'produced', 'analyzed'],
    metrics: [{ unit: 'traffic/leads growth', q: 'Did you grow traffic, leads, or conversions by a measurable amount?' }, { unit: 'campaign ROI/ROAS', q: 'What ROI, ROAS, or engagement did your campaigns achieve?' }],
    overclaims: [{ pattern: /cmo|owned the brand strategy|managed the marketing team/i, warn: 'implies marketing leadership beyond the evidence' }],
    interviewRisks: ['Be ready to defend any channel/metric and walk through a campaign you ran.'],
    employerPriorities: ['measurable results', 'channel fluency', 'clear communication', 'creativity with data'],
  },
  education: {
    label: 'Education / Teaching',
    titles: ['teacher', 'educator', 'instructor', 'tutor', 'professor', 'faculty', 'teaching', 'curriculum', 'paraprofessional', 'teaching assistant'],
    keywords: ['curriculum design', 'lesson planning', 'classroom management', 'differentiated instruction', 'assessment', 'student', 'grading', 'iep', 'pedagogy'],
    provides: ['curriculum design', 'communication', 'classroom management', 'documentation', 'leadership', 'coaching'],
    strongVerbs: ['taught', 'designed', 'developed', 'assessed', 'mentored', 'led', 'improved'],
    metrics: [{ unit: 'students taught', q: 'How many students did you teach or support?' }, { unit: 'score/outcome improvement', q: 'Did test scores, attendance, or outcomes improve measurably?' }],
    overclaims: [{ pattern: /principal|department chair|led the district/i, warn: 'implies administrative leadership beyond a teaching role' }],
    interviewRisks: ['Expect questions on classroom management and adapting instruction for different learners.'],
    employerPriorities: ['student outcomes', 'classroom management', 'communication with parents', 'reliability'],
  },
  project_management: {
    label: 'Project Management',
    titles: ['project manager', 'program manager', 'scrum master', 'project coordinator', 'delivery manager', 'pmo'],
    keywords: ['project management', 'agile', 'scrum', 'stakeholder', 'roadmap', 'jira', 'budget', 'timeline', 'risk management', 'kanban'],
    provides: ['project management', 'coordination', 'communication', 'operations', 'reporting'],
    strongVerbs: ['delivered', 'coordinated', 'led', 'managed', 'planned', 'tracked', 'aligned'],
    metrics: [{ unit: 'projects delivered', q: 'How many projects did you deliver, and on-time/on-budget?' }, { unit: 'team/budget size', q: 'What team size or budget did you coordinate?' }],
    overclaims: [{ pattern: /owned the p&l|vp of|ran the whole program alone/i, warn: 'implies executive ownership beyond the evidence' }],
    interviewRisks: ['Be ready to walk through a project you drove end-to-end and how you handled a slip.'],
    employerPriorities: ['delivery', 'stakeholder communication', 'organization', 'risk awareness'],
  },
  skilled_trades: {
    label: 'Skilled Trades',
    titles: ['electrician', 'plumber', 'hvac', 'welder', 'carpenter', 'machinist', 'mechanic', 'technician', 'journeyman', 'apprentice', 'maintenance'],
    keywords: ['installation', 'repair', 'blueprint reading', 'troubleshooting', 'safety', 'code', 'maintenance', 'preventive maintenance', 'hand tools', 'osha'],
    provides: ['installation', 'troubleshooting', 'safety', 'blueprint reading', 'maintenance', 'reliability'],
    strongVerbs: ['installed', 'repaired', 'maintained', 'troubleshot', 'fabricated', 'inspected', 'wired'],
    metrics: [{ unit: 'jobs/units completed', q: 'How many jobs, installs, or units did you complete?' }, { unit: 'safety record', q: 'Any safety record or code-compliance track record?' }],
    overclaims: [{ pattern: /master (electrician|plumber)|foreman|ran the crew/i, warn: 'implies a license/lead level — verify any license claim' }],
    interviewRisks: ['Expect hands-on and safety/code questions; only claim licenses you actually hold.'],
    employerPriorities: ['quality workmanship', 'safety', 'reliability', 'code compliance'],
  },
};

// Fallback "family" for roles/industries the ontology has not explicitly modeled. It carries
// only genuinely-universal transferable skills and neutral (non-banned) phrasing, so the
// engine still produces safe, specific output for ANY job instead of degrading to nonsense.
export const GENERAL_FAMILY = {
  label: 'General',
  titles: [],
  keywords: [],
  provides: ['communication', 'teamwork', 'problem solving', 'reliability', 'time management'],
  strongVerbs: ['handled', 'built', 'managed', 'improved', 'coordinated', 'delivered', 'supported'],
  metrics: [{ unit: 'volume/outcome', q: 'What was the volume or measurable outcome of your main work (count, %, $, or timeframe)?' }],
  overclaims: [{ pattern: /\b(managed|led|directed|owned)\b/i, warn: 'implies leadership/ownership — make sure the résumé backs it' }],
  interviewRisks: ['Be ready to explain your main responsibilities with a concrete example.'],
  employerPriorities: ['reliability', 'clear communication', 'follow-through'],
};
...
export function detectRoleFamily(title = '', text = '') {
  ...
  // Unknown industry / weak signal → the GENERAL family (universal transferable skills), never
  // a wrong specific family. Require a minimum score so we don't mislabel a marketing résumé
  // as "warehouse" off one stray token.
  if (!ranked.length || ranked[0][1] < 1.2) return { family: 'general', confidence: 0.3, label: 'General' };
...
export function roleFamily(fam) {
  if (fam === 'general') return GENERAL_FAMILY;
