...
// The ontology can never enumerate every skill in every industry. These helpers let ANY two
// skill phrases be compared by their words, so "SEO" matches "SEO", "content strategy" matches
// "content marketing", and "NEC code" matches "NEC code" — with no dictionary entry required.

// Ultra-generic tokens that must NOT, on their own, make two different skills look related.
const GENERIC_TOKENS = new Set(['management', 'skills', 'experience', 'ability', 'knowledge', 'work', 'general', 'strong', 'proficient', 'advanced', 'basic', 'level', 'using', 'related', 'various']);
const STOP = new Set(['and', 'or', 'the', 'a', 'an', 'of', 'in', 'with', 'for', 'to', 'on', 'at', 'by', 'as', 'years', 'year']);

// Whole-token phrase search — avoids "register" matching inside "Registered", "pos" inside
// "position", etc. Handles multi-word and punctuated phrases. Case-insensitive on a lowercased
// haystack (pass lowercased text).
export function hasWord(haystack, phrase) {
  ...
  while (true) {
    ...
    const after = idx + phrase.length >= haystack.length ? ' ' : haystack[idx + phrase.length];
    const okBefore = !/[a-z0-9]/i.test(before) || !/[a-z0-9]/i.test(phrase[0]);
    const okAfter = !/[a-z0-9]/i.test(after) || !/[a-z0-9]/i.test(phrase[phrase.length - 1]);
...
export function normForMatch(s) {
  return String(s || '').toLowerCase()
    .replace(/[^a-z0-9+#. ]/g, ' ')
    .replace(/\b([a-z]{4,})s\b/g, '$1')   // crude singularize (only 4+ char words)
    .replace(/\s+/g, ' ').trim();
}

function meaningfulTokens(s) {
  return normForMatch(s).split(' ').filter(t => t.length > 1 && !STOP.has(t));
}

// Open-vocabulary relation between two arbitrary skill phrases: 'exact' | 'adjacent' |
// 'transferable' | 'none', by shared meaningful (non-generic) word overlap.
export function openRelation(a, b) {
  const na = normForMatch(a), nb = normForMatch(b);
  if (!na || !nb) return { relation: 'none', weight: 0 };
  if (na === nb) return { relation: 'exact', weight: 1 };
  const ta = meaningfulTokens(a), tb = meaningfulTokens(b);
  if (!ta.length || !tb.length) return { relation: 'none', weight: 0 };
  const setB = new Set(tb);
  const shared = ta.filter(t => setB.has(t));
  const meaningfulShared = shared.filter(t => !GENERIC_TOKENS.has(t));
  if (!meaningfulShared.length) return { relation: 'none', weight: 0 };
  // all significant tokens of the shorter phrase are shared → effectively the same skill
  const ratio = shared.length / Math.min(ta.length, tb.length);
  if (ratio >= 0.99) return { relation: 'exact', weight: 1 };
  if (ratio >= 0.5) return { relation: 'adjacent', weight: 0.7 };
  return { relation: 'transferable', weight: 0.55 };
}
...
export function skillRelation(a, b) {
  ...
  // Open-vocabulary fallback — works for skills the ontology has never heard of.
  return openRelation(a, b);
}
