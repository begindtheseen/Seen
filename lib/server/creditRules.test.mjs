...
test('hasProAccess: comped accounts are Pro regardless of the Stripe-managed flag (migration 051)', () => {
  const now = Date.parse('2026-07-10T12:00:00Z');
  // The exact owner-reported failure: grant revoked by reconcile (pro:false) must no
  // longer strip access when the account is comped.
  assert.equal(rules.hasProAccess({ pro: false, comped: true }, now), true);
  assert.equal(rules.hasProAccess({ comped: true }, now), true);
  assert.equal(rules.hasProAccess({ pro: false, comped: false }, now), false);
  // Pre-migration rows (no comped column) behave exactly as before.
  assert.equal(rules.hasProAccess({ pro: true }, now), true);
  assert.equal(rules.hasProAccess({ pro: false }, now), false);
});

test('stripe.js revoke paths carry the comped guard', () => {
  const src = readFileSync(join(ROOT, 'api', 'stripe.js'), 'utf8');
  // setPro's revoke filter — the single choke point every revoke site routes through.
  assert.match(src, /isPro \? '' : '&comped=eq\.false'/, 'setPro must skip comped rows on revoke');
  // reconcile_all must never even list comped accounts.
  assert.match(src, /pro=eq\.true&comped=eq\.false&stripe_customer_id=not\.is\.null/, 'reconcile_all must exclude comped accounts');
});

test('admin set_pro grant writes the durable comped flag', () => {
  const src = readFileSync(join(ROOT, 'api', 'admin-stats.js'), 'utf8');
  assert.match(src, /pro: !!pro, comped: !!pro/, 'set_pro must set comped alongside pro');
});
