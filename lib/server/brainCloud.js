...
// READ: compact orientation for SessionStart. In gateway mode this avoids transferring the entire
// vault into the cloud container just to render a short briefing.
export async function fetchBriefing(opts = {}) {
  if (useGateway()) return (await gw('briefing', opts)).briefing;
  return store.briefing(opts);
}

export async function searchFactsCloud(filters = {}) {
  if (useGateway()) return gw('search_facts', filters);
  return store.findFacts(filters);
}

export async function contradictionsCloud() {
  if (useGateway()) return (await gw('contradictions')).contradictions;
