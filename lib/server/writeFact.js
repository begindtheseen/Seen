...
export function applyFact(text, noteRel, fact, today = isoToday(), nowTs = new Date().toISOString()) {
  ...
  if (conflicts.length) {
    // Name the ACTUAL colliding pair, not the incoming write. findContradictions scans the WHOLE note,
    // so the conflict is very often a pre-existing pair that has nothing to do with what you are
    // writing — and blaming the incoming subject/predicate sends you debugging the wrong fact. That
    // cost a cloud session an entire investigation on 2026-08-13: the error named its own write while
    // the real collision was a retraction that had never closed the fact it retracted.
    const pairs = conflicts.slice(0, 3).map(({ a, b }) => {
      const win = (f) => `${f.valid_from}->${f.valid_to == null ? 'open' : f.valid_to}`;
      return `${a.subject}/${a.predicate}: ${a.id} (${win(a)}) vs ${b.id} (${win(b)})`;
    }).join(' | ');
    const mine = conflicts.some(({ a, b }) => [a, b].some((f) =>
      String(f.subject) === String(subject) && String(f.predicate) === String(predicate)));
    throw new Error(`write blocked: ${conflicts.length} contradiction(s) in ${noteRel}`
      + `${mine ? '' : ' — PRE-EXISTING, unrelated to this write'}. Colliding: ${pairs}`
      + '. Close the stale side (its valid_to should be the date its replacement began), then retry.');
  }
...
// Repair legacy timeline headers without touching the append-only body. Older cloud writers omitted
// `type: daily`, and one historical day has no frontmatter at all.
export function ensureTimelineFrontmatter(content, date) {
  const cur = String(content || '');
  const header = `---\ntitle: ${date}\ntype: daily\ndate: ${date}\ntags: [timeline]\n---`;
  if (!cur.trim()) return `${header}\n`;
  if (!cur.startsWith('---\n')) return `${header}\n\n${cur}`;
  const end = cur.indexOf('\n---', 4);
  if (end < 0) return cur;
  const lines = cur.slice(4, end).split('\n');
  if (!lines.some((line) => /^type:\s*daily\s*$/i.test(line))) {
    const dateAt = lines.findIndex((line) => /^date:\s*/i.test(line));
    lines.splice(dateAt >= 0 ? dateAt : lines.length, 0, 'type: daily');
  }
  if (!lines.some((line) => /^date:\s*/i.test(line))) lines.push(`date: ${date}`);
  return `---\n${lines.join('\n')}\n---${cur.slice(end + 4)}`;
}
...
export function appendTimeline(vaultDir, date, heading, text) {
  ...
  // Anchor the whole read/repair/write operation to one descriptor. O_NOFOLLOW rejects an existing
  // symlink, and retaining the descriptor prevents the destination from being swapped after a
  // separate existence check (the filesystem race CodeQL correctly flags on path-based writes).
  const flags = constants.O_RDWR | constants.O_CREAT | constants.O_APPEND | (constants.O_NOFOLLOW || 0);
  const fd = openSync(path, flags, 0o644);
  try {
    const cur = ensureTimelineFrontmatter(readFileSync(fd, 'utf8'), date);
