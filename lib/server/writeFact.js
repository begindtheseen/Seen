...
import { parseFrontmatter, normalizeFacts, findContradictions } from './memoryGraph.js';

const FIELD_ORDER = ['id', 'subject', 'predicate', 'object', 'valid_from', 'valid_to', 'confidence', 'source', 'recorded', 'invalidated'];
...
function emitVal(v) {
  ...
  const s = String(v);
  // Quote if it contains YAML-significant chars or has edge whitespace; otherwise emit bare.
...
export function applyFact(text, noteRel, fact, today = isoToday()) {
  ...
  next.push({
    ...
    recorded: today, invalidated: null,
  });
