...
test('isRemoteLocation detects remote phrasings, not real places', () => {
  for (const s of ['Remote', 'remote (US)', 'Work from home', 'WFH', 'anywhere']) assert.ok(isRemoteLocation(s), s);
  for (const s of ['Denver, CO', 'New York', '']) assert.equal(isRemoteLocation(s), false, s);
});

test('buildEmployerListingRow sets remote/city so listings hit seeker location filters', () => {
  const opts = { uid: 'u', company: 'Acme', companyKey: 'acme', nowISO: '2026-07-19T00:00:00Z', expiresISO: '2026-09-17T00:00:00Z' };
  const remoteRow = buildEmployerListingRow(validateNewListing({ title: 'Cook', apply_url: 'https://a.com', location: 'Remote' }).listing, opts);
  assert.equal(remoteRow.remote, true);
  assert.equal(remoteRow.city, null);
  const cityRow = buildEmployerListingRow(validateNewListing({ title: 'Cook', apply_url: 'https://a.com', location: 'Denver, CO' }).listing, opts);
  assert.equal(cityRow.remote, false);
  assert.equal(cityRow.city, 'Denver, CO');
  assert.equal(cityRow.location, 'Denver, CO');
  // lat/lng are NOT set by the pure builder — the endpoint geocodes (I/O). Kept out of the pure row.
  assert.equal('lat' in cityRow, false);
});
...
test('buildEmployerListingRow stamps ownership + source Employer', () => {
  ...
  assert.equal('url' in row, false); // the jobs table has apply_url only — no phantom `url` column
