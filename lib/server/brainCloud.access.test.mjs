...
import { cloudConfigured, contradictionsCloud, fetchBriefing, fetchNotes, gatewayConfigured,
  gatewaySelected, searchFactsCloud } from './brainCloud.js';
...
test('shared project MCP config is empty so credentials stay private per principal', () => {
...
test('gateway exposes compact briefing, filtered search, and contradiction reads with exact identity', async () => {
  await withEnvironment({
    BRAIN_API_URL: 'https://gateway.example/api/brain', BRAIN_API_TOKEN: 'outer-test',
    BRAIN_CLIENT_TOKEN: 'client-test', BRAIN_CLIENT: 'claude-seenjobs',
  }, async () => {
    const bodies = [];
    global.fetch = async (_url, init) => {
      const body = JSON.parse(init.body); bodies.push(body);
      const payload = body.op === 'briefing' ? { ok: true, briefing: { counts: {}, changed: { added: [], invalidated: [] }, openThreads: [], contradictions: [], lowConfidence: [] } }
        : body.op === 'search_facts' ? { ok: true, facts: [{ id: 'f1' }], count: 1, truncated: false }
          : { ok: true, contradictions: { count: 0, conflicts: [], truncated: false } };
      return { ok: true, status: 200, text: async () => JSON.stringify(payload) };
    };
    assert.deepEqual((await fetchBriefing({ today: '2026-08-14' })).counts, {});
    assert.deepEqual((await searchFactsCloud({ subject: 'Seen' })).facts, [{ id: 'f1' }]);
    assert.equal((await contradictionsCloud()).count, 0);
    assert.deepEqual(bodies, [
      { op: 'briefing', by: 'claude-seenjobs', today: '2026-08-14' },
      { op: 'search_facts', by: 'claude-seenjobs', subject: 'Seen' },
      { op: 'contradictions', by: 'claude-seenjobs' },
    ]);
  });
});
