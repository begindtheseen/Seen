...
// ── Bot-wall recovery ladder ($0: indeed alternates + jina reader) ───────────

test('indeedJobKey extracts the jk from every Indeed URL shape', () => {
  assert.equal(indeedJobKey('https://www.indeed.com/viewjob?jk=ef3a8f8e35490b85&from=appshareios'), 'ef3a8f8e35490b85');
  assert.equal(indeedJobKey('https://www.indeed.com/m/viewjob?jk=AB12CD34EF56AB78'), 'ab12cd34ef56ab78');
  assert.equal(indeedJobKey('https://www.indeed.com/rc/clk?jk=ef3a8f8e35490b85'), 'ef3a8f8e35490b85');
  assert.equal(indeedJobKey('https://uk.indeed.com/viewjob?vjk=ef3a8f8e35490b85'), 'ef3a8f8e35490b85');
  assert.equal(indeedJobKey('https://www.indeed.com/job/line-cook-ef3a8f8e35490b85'), 'ef3a8f8e35490b85');
  assert.equal(indeedJobKey('https://www.linkedin.com/jobs/view/1'), null);
});

test('alternateFetchPlans: Indeed gets mobile → jina → rpc; other hosts get jina', () => {
  const indeed = alternateFetchPlans('https://www.indeed.com/viewjob?jk=ef3a8f8e35490b85');
  assert.deepEqual(indeed.map(p => p.kind), ['html', 'jina', 'indeed-rpc']);
  assert.equal(indeed[0].url, 'https://www.indeed.com/m/viewjob?jk=ef3a8f8e35490b85');
  assert.ok(indeed[1].url.startsWith('https://r.jina.ai/https://www.indeed.com/'));
  assert.equal(indeed[2].url, 'https://www.indeed.com/rpc/jobdescs?jks=ef3a8f8e35490b85');
  const li = alternateFetchPlans('https://www.linkedin.com/jobs/view/x-at-y-1');
  assert.deepEqual(li.map(p => p.kind), ['jina']);
});

test('indeedEmbeddedFields pulls listing data out of Indeed page JSON', () => {
  const html = `<script>window._initialData=JSON.parse('{}');var x={"jobInfoModel":{"jobTitle":"Warehouse Associate","companyName":"Southwest Logistics","formattedLocation":"Dallas, TX","sanitizedJobDescription":"\\u003cp\\u003ePick, pack, and ship orders. Pay $19\\/hr. Benefits include 401k.\\u003c\\/p\\u003e"}};</script>`;
  const f = indeedEmbeddedFields(html);
  assert.equal(f.title, 'Warehouse Associate');
  assert.equal(f.company, 'Southwest Logistics');
  assert.equal(f.location, 'Dallas, TX');
  assert.ok(f.description.includes('Pick, pack'));
  assert.equal(indeedEmbeddedFields('<html>captcha wall</html>'), null);
});

test('parseJinaReader parses the reader output into fields', () => {
  const text = `Title: Line Cook - Portland, OR - Acme Corp | Indeed.com
URL Source: https://www.indeed.com/viewjob?jk=abc
Markdown Content:
# Line Cook

**Acme Corp** · Portland, OR

Cook on the line at our flagship restaurant. [Apply now](https://x.com) Full training, 401k, PTO. Two interview rounds.`;
  const f = parseJinaReader(text, 'https://www.indeed.com/viewjob?jk=abc');
  assert.equal(f.title, 'Line Cook');
  assert.equal(f.company, 'Acme Corp');
  assert.equal(f.location, 'Portland, OR');
  assert.ok(f.description.includes('flagship restaurant'));
  assert.ok(!f.description.includes(']('), 'markdown links stripped');
  assert.equal(f.via, 'jina');
  assert.equal(parseJinaReader('', 'https://x.com'), null);
});

test('challenge pages ("Just a moment…") can never become a listing — verified live via jina', () => {
  // parseListingTitle refuses challenge titles outright.
  const p = parseListingTitle('Just a moment...');
  assert.equal(p.title, '');
  // A jina response that got challenged yields null, not a garbage listing.
  const challenged = `Title: Just a moment...\nURL Source: https://www.indeed.com/viewjob?jk=x\nMarkdown Content:\nChecking your browser before accessing indeed.com`;
  assert.equal(parseJinaReader(challenged, 'https://www.indeed.com/viewjob?jk=x'), null);
  // And the HTML extractor's title fallback skips them too.
  const html = `<head><title>Attention Required! | Cloudflare</title></head>`;
  assert.equal(extractListingFromHtml(html, 'https://www.indeed.com/viewjob?jk=x').title, '');
});

// ── Anti-blocking hygiene: realistic, consistent, rotating browser headers ────

test('browserHeaders builds a complete, self-consistent browser header set', () => {
  const h = browserHeaders('https://www.indeed.com/viewjob?jk=abc', { seed: 0 });
  assert.ok(h['User-Agent'].includes('Mozilla/5.0'));
  assert.equal(h['Accept-Language'], 'en-US,en;q=0.9');
  assert.equal(h['Sec-Fetch-Mode'], 'navigate');
  // Same-host Referer (a real navigation presents one).
  assert.equal(h.Referer, 'https://www.indeed.com/');
  assert.equal(h['Sec-Fetch-Site'], 'same-origin');
  // Chrome profiles carry a matching Sec-CH-UA + platform; Safari/Firefox omit it.
  for (const p of BROWSER_PROFILES) {
    if (/Chrome\//.test(p['User-Agent']) && !/Firefox/.test(p['User-Agent'])) {
      assert.ok(p['sec-ch-ua'] && p['sec-ch-ua-platform'], 'chrome profile has client hints');
    } else {
      assert.ok(!p['sec-ch-ua'], 'non-chrome profile correctly omits sec-ch-ua');
    }
  }
});

test('browserHeaders rotates the profile across seeds and forces mobile when asked', () => {
  const uas = new Set([0, 1, 2, 3].map(seed => browserHeaders('https://x.com/j', { seed })['User-Agent']));
  assert.ok(uas.size >= 2, 'successive requests vary the UA');
  const mobile = browserHeaders('https://www.indeed.com/m/viewjob?jk=1', { mobile: true });
  assert.ok(/iPhone/.test(mobile['User-Agent']), 'mobile flag selects the iPhone profile');
});

test('browserHeaders never emits a Chrome UA without its matching client hints (anti-fingerprint mismatch)', () => {
  for (let seed = 0; seed < 12; seed++) {
    const h = browserHeaders('https://x.com/j', { seed });
    const isChrome = /Chrome\//.test(h['User-Agent']) && !/Firefox/.test(h['User-Agent']);
    if (isChrome) assert.ok(h['sec-ch-ua-platform'], `chrome seed ${seed} must carry platform hint`);
    else assert.ok(!h['sec-ch-ua'], `non-chrome seed ${seed} must not carry sec-ch-ua`);
  }
});
