...
export async function gateAI(req, reason = 'ai_tool', { proOnly = false } = {}) {
  ...
  if (!cRes.ok) return { ok: true, uid, balance: 1, pro: false };
  ...
  // Daily reset: 1 credit/day (matches consume_credit RPC, migration 031). Keeps the free
  // tier lean so users upgrade after the welcome credits run out.
  if (cred.last_reset !== today) {
    cred = { ...cred, balance: 1, daily_earned: 0, last_reset: today };
    await db(`ai_credits?user_id=eq.${uid}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ balance: 1, daily_earned: 0, last_reset: today }) });
  }
