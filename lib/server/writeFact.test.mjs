...
import { constants, mkdtempSync, mkdirSync, rmSync, readFileSync, symlinkSync, writeFileSync } from 'node:fs';
...
test('appendTimeline refuses to follow a symlink destination', { skip: !constants.O_NOFOLLOW }, () => {
  const dir = mkdtempSync(join(tmpdir(), 'seen-timeline-'));
  try {
    mkdirSync(join(dir, 'timeline'));
    const outside = join(dir, 'outside.md');
    writeFileSync(outside, 'must stay unchanged\n');
    symlinkSync(outside, join(dir, 'timeline', '2026-08-14.md'));

    assert.throws(
      () => appendTimeline(dir, '2026-08-14', 'Blocked', 'must not be written'),
