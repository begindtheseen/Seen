...
// NEWEST FIRST, always. Index order is note-path order, so an unsorted list handed to a capped
// consumer (the briefing caps it) dropped whichever changes happened to live in a late-sorting note —
// routinely the most recent ones, the exact thing a briefing exists to report. Sorting here means
// every cap, present or future, can only ever drop the OLDEST changes.
...
export function whatsChanged(facts, since) {
  const added = facts.filter((f) => f.recorded && f.recorded >= since).sort(newestFirst('recorded'));
  const invalidated = facts.filter((f) => f.invalidated && f.invalidated >= since).sort(newestFirst('invalidated'));
