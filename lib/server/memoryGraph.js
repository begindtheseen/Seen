...
const FACT_FIELDS = ['id', 'subject', 'predicate', 'object', 'valid_from', 'valid_to',
  'confidence', 'source', 'recorded', 'invalidated'];
...
// Parse the fixed timeline note sections into an episode. Timeline notes carry
// `type: daily` + `date:`; the body has `## Shipped`, `## Verified ...`, `## Decisions`, etc.
export function parseTimelineEpisode(data, body, notePath) {
  if (!data || data.type !== 'daily' || !data.date) return null;
  const sections = {};
  const re = /^##\s+(.+)$/gm;
  let m; const marks = [];
...
export function buildIndex(notes) {
  ...
  return { facts, episodes, edges, entities, errors };
}
