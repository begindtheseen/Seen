...
export function buildImportedJobRow({ title, company, location, salary, salaryMin, description, type }, url) {
  ...
  // Score only when the listing carries enough of its own signal to score honestly
  // ("score for a reason or no score") — a bare title+company import stays unrated.
  if (desc.length >= 80) {
