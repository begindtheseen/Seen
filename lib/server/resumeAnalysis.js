...
const STOPWORDS = new Set([
  ...
  // Benefits / EEO / application-process boilerplate. Every posting carries it and
  // none of it is a skill, so surfacing it as a "keyword to add" is pure noise —
  // "Associates are offered medical, dental, vision and a 401k" was yielding
  // "associates" and "eligible" as things to put on a résumé.
  'associate','associates','medical','dental','vision','insurance','401k','pto',
  'paid','holiday','holidays','weekend','weekends','vacation','eligible',
  'eligibility','wage','wages','salary','hourly','pay','discount','discounts',
  'retirement','healthcare','health','online','application','applications',
  'resume','interview','hire','hires','hiring','onboarding','drug','screen',
  'screening','background','check','checks',
  // JD grammar and adverbs — they describe how a duty is performed, never a skill.
  'responsible','repeatedly','properly','accurately','consistently','effectively',
  'efficiently','regularly','frequently','occasionally','independently',
  'preferably','approximately','successfully','primarily','typically',
]);
...
// Two words are about the same subject when they agree from at least five characters
// in — "volume"/"volumes", "accuracy"/"accurate", "warehouse"/"warehousing". Shorter
// words must match outright, or unrelated stubs would collide.
...
// The missing keyword this SPECIFIC bullet is already about, or null. A keyword only
// belongs in a bullet that covers the same subject: "Safely operated ... high volumes"
// can honestly carry "high-volume"; a bullet about torque specifications cannot carry
// "pallet". Gaps with no such anchor belong in the gap list, not stapled to a line
// they have nothing to do with.
function relatedMissingKeyword(text, missing) {
  // Compound terms are split on their joiners: tokenize() keeps "high-volume" whole, so
  // without this the keyword never meets the bullet's own word "volumes".
  const subWords = s => tokenize(s).flatMap(w => w.split(/[-/.]+/)).filter(w => w.length >= 4);
  const words = subWords(text);
  for (const k of missing || []) {
    const parts = subWords(k.term);
    if (!parts.length) continue;
    if (parts.every(p => words.includes(p))) continue;   // already in this bullet
    if (parts.some(p => words.some(w => shareStem(w, p)))) return k;
  }
  return null;
}
...
function bulletNote(candidate, topMissing) {
  ...
  // Suggest a missing keyword only when THIS bullet is already about that subject.
  // The previous version handed every bullet topMissing[0], so a six-bullet résumé got
  // six identical "weave in pallet" notes — one of them on a line about torque specs.
  const kw = relatedMissingKeyword(candidate.text, topMissing);
  if (kw) {
...
export function runOptimize({ resume, jobDescription, job, company, pro }) {
  ...
  // Wire field name is `keywords_added` for backward compatibility, but nothing is ever
  // inserted into the candidate's text — these are terms the résumé does NOT contain.
  // The UI renders them as gaps ("Add if you genuinely have it"). Do not read the name
  // as a claim that the résumé was changed.
  ...
  for (let n = 0; n < targets.length; n++) {
    const t = targets[n];
    // What this bullet ACTUALLY demonstrates: the highest-ranked posting keyword the
    // bullet itself contains. This used to be priorities[n % priorities.length] — a
    // round-robin label that told the reader a bullet about engine repairs "addresses
    // pallet". Left empty when the bullet matches nothing, rather than guessed; the UI
    // filters empties out of its match list.
    const bulletTokens = tokenSet(t.text);
    const bulletLower = t.text.toLowerCase();
    const priority = ranked.find(k => resumeHasKeyword(k, bulletLower, bulletTokens))?.display || '';
...
// Employment type / arrangement tokens that résumés append after a company. These are
// never an employer name, so a line made only of them ("Self-Employed | Part-Time")
// supplements the role header above it rather than opening a new entry.
const EMPLOYMENT_TYPE_RE = /\b(temp(?:orary)?|contract(?:or)?|full[\s-]?time|part[\s-]?time|seasonal|intern(?:ship)?|freelance(?:r)?|self[\s-]?employed|independent\s+contractor|per[\s-]?diem|remote|hybrid|on-?site|volunteer|co-?op|w2|1099|c2h|contract\s+to\s+hire)\b/i;

// A segment that is a work LOCATION or employment-type/date META, not a company name —
// so it never gets mistaken for the employer (which fragments the company page).
function isLocationOrMeta(s) {
  const t = String(s || '').trim();
  if (!t) return true;
  if (/,\s*[A-Z]{2}\b/.test(t)) return true;                          // "La Jolla, CA"
  if (EMPLOYMENT_TYPE_RE.test(t) && t.split(/\s+/).length <= 3) return true; // "Temp contract"
  if (DATE_RANGE_RE.test(t) || /^\W*(19|20)\d\d\b/.test(t)) return true;     // dates
  return false;
}

// Reduce an extracted company field to just the employer name — strip an appended
// location, employment type, dates, and separators. "Aptera — La Jolla, CA · Temp
// contract" → "Aptera". Keeps intel routed to the real company when people search it.
function cleanCompany(raw) {
  let c = String(raw || '').trim();
  // Company is the part before a bullet/pipe separator.
  c = c.split(/\s*[·•|]\s*/)[0].trim();
  // Drop a trailing "— City, ST" / "- City, ST" location.
  c = c.replace(/\s*[—–-]\s*[A-Z][A-Za-z.'’ ]*,\s*[A-Z]{2}\b.*$/, '').trim();
  // Drop a trailing ", City, ST" / ", City" / ", ST".
  c = c.replace(/,\s*[A-Z][A-Za-z.'’]*(?:\s+[A-Z][A-Za-z.'’]*){0,2}(?:,\s*[A-Z]{2})?\s*$/, '').trim();
  // Drop trailing employment-type words + any dangling separators/punctuation.
  c = c.replace(new RegExp(`[\\s,·•|–—-]*${EMPLOYMENT_TYPE_RE.source}\\.?\\s*$`, 'i'), '').trim();
  c = c.replace(/[·•|,\-–—\s]+$/, '').trim();
  return c;
}

// ── Section-aware experience segmentation ────────────────────────────────────
// The résumé is read in ONE pass: section headers switch context, role headers open
// an entry, and every line after a header belongs to that entry until the next one.
//
// This replaced a two-part scheme that lost real content on ordinary résumés. The old
// parser created an entry only where it found a DATE RANGE, then re-located each
// entry's bullets by fuzzy substring match on the company name. Both halves failed:
//   • A role with no date range — freelance work, a tail list of older jobs — never
//     became an entry, so its lines were swallowed as bullets of the job above it.
//   • Matching bullets by company substring anchored an entry to the FIRST line that
//     merely contained the name. On a résumé where the employer had been misread as the
//     word "EXPERIENCE" (the section header, picked up as a company by the two-lines-up
//     fallback), the entry latched onto the summary paragraph, hit the real EXPERIENCE
//     header one line later, and every one of its bullets was dropped from the export.
// Reading sections is what makes the date requirement unnecessary inside an explicit
// EXPERIENCE block, and what keeps Education/Skills text out of the bullet lists.

// Section names that introduce work history (as opposed to education/skills/etc.).
const EXPERIENCE_SECTION_RE = /^(experience|work experience|professional experience|employment|employment history|work history|relevant experience|career history)\b/i;

// Strip the decoration résumé exporters wrap around headers — leading/trailing runs
// of = - * _ ~ # • and a trailing colon — so "=== EXPERIENCE ===" reduces to the word.
...
// 'experience' | 'other' when the line is a section header, else null.
function sectionKindOf(line) {
  if (!isSectionHeader(line)) return null;
  return EXPERIENCE_SECTION_RE.test(stripHeaderDecoration(line)) ? 'experience' : 'other';
}

const BULLET_GLYPH_RE = /^\s*[•\-*–·●▪◦‣>]\s+/;

// Résumé bullets overwhelmingly open with a past-tense or gerund action verb
// ("Led", "Provided", "Managing"). Role headers never do. This is what stops a bullet
// that carries no glyph — common after PDF extraction — from being read as the next
// job's header. It is only consulted AFTER looksLikeTitle, so a real title that happens
// to start with an -ing word ("Marketing Manager", "Operating Engineer") is unaffected.
const BULLET_OPENER_RE = /^(?:successfully|effectively|safely|consistently|independently|actively|routinely|directly)?\s*(?:[a-z]+(?:ed|ing)|responsible for|led|built|ran|drove|grew|sold|wrote|made|took|kept|met|won|oversaw|served|spoke|taught)\b/i;

// Long or sentence-shaped lines are prose, not headers. Lines carrying a date range are
// exempt at the call site — "Bliss Car Wash — CSR    Jan 2022 - Present" is long but is
// unambiguously a header.
...
// Does this line OPEN a role entry, as opposed to describing one?
function isRoleHeaderLine(line) {
  const t = String(line || '').trim();
  if (!t) return false;
  if (BULLET_GLYPH_RE.test(t)) return false;      // an explicit bullet is never a header
  if (isContactLine(t)) return false;
  if (DATE_RANGE_RE.test(t)) return true;         // a date range always marks a header
  if (looksLikeTitle(t)) return true;
  if (BULLET_OPENER_RE.test(t)) return false;
  if (looksLikeProse(t)) return false;
  return isProbablyCompany(t);
}

// Read {title, company, dates} out of ONE header line. `orientation` resolves lines
// where neither side names a recognisable role — see detectHeaderOrientation.
function readHeaderLine(line, orientation = 'company-first') {
  const t = String(line || '').trim().replace(BULLET_GLYPH_RE, '');
  const dm = t.match(DATE_RANGE_RE);
  const out = {
    title: '', company: '', meta: false,
    start_date: dm ? dm[1] : '',
    end_date: dm ? (/present|current|now/i.test(dm[2]) ? 'Present' : dm[2]) : '',
  };
  const carrier = t.replace(DATE_RANGE_RE, '')
    .replace(/^[\s|·•,;–—-]+/, '').replace(/[\s|·•,;–—-]+$/, '').trim();
  if (!carrier) { out.meta = true; return out; }

  // Each real field (title, company, location, dates, type) is its own segment, so the
  // company is a SINGLE segment — never company+location+type run together.
  const segs = carrier
    .split(/\s+(?:at|@|[—–|·•,])\s+|\s+-\s+/i)
    .map(s => s.trim().replace(/^[•\-*]\s*/, ''))
    .filter(Boolean);
  const real = segs.filter(s => !isLocationOrMeta(s));

  if (!real.length) { out.meta = true; return out; }

  const titleIdx = segs.findIndex(s => looksLikeTitle(s));
  if (titleIdx !== -1) {
    out.title = segs[titleIdx];
    const co = segs.find((s, i) => i !== titleIdx && isProbablyCompany(s) && !looksLikeTitle(s) && !isLocationOrMeta(s));
    if (co) out.company = co;
  } else if (real.length >= 2) {
    if (orientation === 'title-first') { out.title = real[0]; out.company = real[1]; }
    else { out.company = real[0]; out.title = real[1]; }
  } else if (isProbablyCompany(real[0])) {
    out.company = real[0];
  } else {
    out.title = real[0];
  }

  out.title = out.title.slice(0, 100);
  out.company = out.company ? cleanCompany(out.company).slice(0, 100) : '';
  if (!out.title && !out.company) out.meta = true;
  return out;
}

// Résumé layouts are internally consistent: a document writes either "Title — Company"
// throughout or "Company — Title" throughout. Count the headers we CAN resolve (exactly
// one side names a recognisable role) and apply the winning orientation to the ones we
// can't — that is how "Team Member – Chipotle" resolves without adding every job noun in
// the language to the title dictionary. Defaults to company-first, the prior behaviour.
function detectHeaderOrientation(headerLines) {
  let titleFirst = 0, companyFirst = 0;
  for (const line of headerLines) {
    const carrier = String(line).replace(DATE_RANGE_RE, '').trim();
    const segs = carrier
      .split(/\s+(?:at|@|[—–|·•,])\s+|\s+-\s+/i)
      .map(s => s.trim().replace(/^[•\-*]\s*/, ''))
      .filter(Boolean);
    if (segs.length < 2) continue;
    const tIdx = segs.findIndex(s => looksLikeTitle(s));
    if (tIdx === -1) continue;
    const cIdx = segs.findIndex((s, i) => i !== tIdx && isProbablyCompany(s) && !looksLikeTitle(s) && !isLocationOrMeta(s));
    if (cIdx === -1) continue;
    if (tIdx < cIdx) titleFirst++; else companyFirst++;
  }
  return titleFirst > companyFirst ? 'title-first' : 'company-first';
}

// Résumés often list short older roles on ONE line, pipe-separated:
//   "Campaign Manager – Victoriam Management | Team Member – Chipotle"
// Split only when at least two pipe-separated pieces EACH resolve to a title AND a
// company; otherwise the line stays whole ("Valet Attendant | Oct 2022 – Jan 2025").
// This runs BEFORE isRoleHeaderLine, because a line packing four roles is long enough
// to read as prose — which is exactly how four real jobs became one bullet in an export.
// Completeness is orientation-independent (a two-segment piece resolves to a title and a
// company under either orientation), so the vote pass can split with the default.
function splitMultiRoleLine(line, orientation) {
  const pieces = String(line).split(/\s+[|·•]\s+/).map(s => s.trim()).filter(Boolean);
  if (pieces.length < 2) return [line];
  const complete = pieces.filter(p => { const r = readHeaderLine(p, orientation); return r.title && r.company; });
  return complete.length >= 2 ? pieces : [line];
}

// Walk the résumé once and return blocks of { header: string[], bullets: string[],
// section }. A block stays open across supplementary header lines (a bare date line, an
// employment-type line, the title line under a company line) and closes at the next real
// role header or section header.
function segmentExperienceBlocks(resumeText) {
  const raw = String(resumeText || '').split(/\r?\n/).map(l => l.trim());

  // Orientation is a whole-document property, so resolve it before segmenting. Only
  // lines inside an experience section vote — an education line ("Great Oak High School
  // – Temecula, CA") is not evidence about how this résumé orders job headers.
  const votes = [];
  {
    let section = null;
    for (const line of raw) {
      ...
      for (const part of splitMultiRoleLine(line, 'company-first')) {
        if (isRoleHeaderLine(part)) votes.push(part);
      }
    }
  }
  const orientation = detectHeaderOrientation(votes);

  const blocks = [];
  let section = null;
  let cur = null;
  const close = () => { if (cur) blocks.push(cur); cur = null; };
  const open = (line, index) => { cur = { header: [line], bullets: [], section, index }; };
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    const kind = sectionKindOf(line);
    if (kind) { close(); section = kind; continue; }
    if (section === 'other') continue;                     // education / skills / summary
    if (isContactLine(line) && !DATE_RANGE_RE.test(line)) continue;

    // A line that splits into several complete role headers IS a header run, whatever
    // its length; otherwise fall back to classifying the line as written.
    const parts = splitMultiRoleLine(line, orientation);
    if (parts.length === 1 && !isRoleHeaderLine(line)) {
      if (cur) cur.bullets.push(line.replace(BULLET_GLYPH_RE, '').trim());
      continue;                                            // stray prose before any entry
    }

    for (const part of parts) {
      if (!cur) { open(part, i); continue; }
      // A header line while a block is open either SUPPLEMENTS that block's header or
      // starts the next entry. Once a block has bullets, the header run is over.
      if (cur.bullets.length === 0 && isHeaderSupplement(part, cur, orientation)) { cur.header.push(part); continue; }
      close();
      open(part, i);
    }
  }
...
// True when `line` adds to the open block's header rather than starting the next entry.
function isHeaderSupplement(line, block, orientation) {
  const r = readHeaderLine(line, orientation);
  if (r.meta) return true;                       // "2020 – Present", "Self-Employed | Part-Time"
  if (r.title && r.company) return false;        // a complete header — this is the next job
  const have = readBlockHeader(block, orientation);
  if (r.title && !have.title) return true;       // "Towne Park" then "Valet Attendant | dates"
  if (r.company && !have.company && !r.title) return true;
  return false;
}

// Fold a block's header lines into one {title, company, start_date, end_date}. The first
// non-empty value for each field wins, so the primary header line takes precedence over
// its supplements.
function readBlockHeader(block, orientation) {
  const out = { title: '', company: '', start_date: '', end_date: '' };
  for (const line of block.header) {
    const r = readHeaderLine(line, orientation);
    if (!out.title && r.title) out.title = r.title;
    if (!out.company && r.company) out.company = r.company;
    if (!out.start_date && r.start_date) { out.start_date = r.start_date; out.end_date = r.end_date; }
  }
  return out;
}

// Parse the résumé into employment entries WITH the bullets that belong to each one.
// This is the single source of truth for both extractEmployment() (company intel,
// tenure) and buildResumeDocument() (the exported PDF) — they used to segment the
// résumé differently, which is how the export could contain roles the intel never saw.
export function parseExperienceEntries(resumeText) {
  const { blocks, orientation } = segmentExperienceBlocks(resumeText);
  const entries = [];

  for (const block of blocks) {
    if (entries.length >= 15) break;
    const h = readBlockHeader(block, orientation);
    if (!h.title && !h.company) continue;
    // Outside an explicit EXPERIENCE section we have no structural evidence that a short
    // capitalised line is a job at all — the name and tagline at the top of every résumé
    // look identical. There, keep requiring a date range, as the parser always has.
    if (block.section !== 'experience' && !h.start_date) continue;

    const bullets = block.bullets.filter(b => {
      if (!b || isContactLine(b)) return false;
...
// Extract up to 15 employment entries for company intel / tenure. Entries with no
// identifiable employer are dropped here (they cannot route intel anywhere) — the
// résumé EXPORT keeps them, because a role with no named employer is still the
// candidate's real work history.
export function extractEmployment(resumeText) {
  const seen = new Set();
  return parseExperienceEntries(resumeText)
    .map(({ company, title, start_date, end_date }) => ({ company, title, start_date, end_date }))
    .filter(e => {
...
function isSectionHeader(line) {
  // Strip decoration that résumé exporters wrap around headers ("=== EXPERIENCE ===",
  // "* Skills *", "--- Education ---", "## Summary") down to the bare header word.
  const s = stripHeaderDecoration(line);
  if (!s) return false;
...
export function buildResumeDocument({ resume, jobDescription, job, company }) {
  ...
  // One segmentation feeds both the exported document and the employment history, so
  // the PDF can never contain a role the rest of the system does not know about.
  let parsed = [];
  try { parsed = parseExperienceEntries(text); } catch (_) { parsed = []; }
  const employment = parsed.filter(e => e.company)
    .map(({ company, title, start_date, end_date }) => ({ company, title, start_date, end_date }));
