...
export const EMPLOYER_SKUS = {
  ...
  sponsor30: {
    ...
    // ⚠ DEFAULT PRICE — owner to confirm. Sponsorship is an owner-approved revenue line (Operation
    // 50%, employer-side); this extends the LOCKED one-time model with another one-time SKU (it does
    // NOT introduce recurring billing). $149 sits between Featured ($79) and Verified ($249): a
    // single, exclusive brand slot on the beachhead hero asset (/agencies). Change this one literal.
    amount_cents: 14900, // $149  ← adjust to the real number
    days: 30,
    kind: 'sponsor',
