...
import {
  ...
  disputeStatusCounts, orderDisputesOpenFirst, buildListingTickets,
} from './listingDisputes.js';

test('buildListingTickets unifies a seeker report and an employer dispute on ONE listing', () => {
  const tickets = buildListingTickets({
    reports: [
      { job_id: 'j1', status: 'expired', reported_at: '2026-07-10T00:00:00Z', title: 'Cook', company: 'Acme', apply_url: 'https://a/1' },
      { job_id: 'j1', status: 'unknown', reported_at: '2026-07-11T00:00:00Z' },
    ],
    disputes: [
      { id: 7, job_id: 'j1', company_name: 'Acme', kind: 'still_active', detail: 'we interviewed yesterday', status: 'open', created_at: '2026-07-12T00:00:00Z' },
    ],
    jobsById: { j1: { id: 'j1', title: 'Cook', company: 'Acme', apply_url: 'https://a/1', availability_status: 'active' } },
  });
  assert.equal(tickets.length, 1); // ONE ticket, not a report + a dispute separately
  const t = tickets[0];
  assert.equal(t.job_id, 'j1');
  assert.equal(t.dispute.id, 7);
  assert.equal(t.reports.count, 2);
  assert.equal(t.reports.expired, 1);
  assert.equal(t.status, 'open');
});

test('buildListingTickets: a report with no dispute is still a ticket; a removed listing resolves', () => {
  const tickets = buildListingTickets({
    reports: [{ job_id: 'j2', status: 'expired', reported_at: '2026-07-10T00:00:00Z', apply_url: 'https://a/2', title: 'Server', company: 'Acme' }],
    disputes: [],
    jobsById: { j2: { id: 'j2', title: 'Server', company: 'Acme', apply_url: 'https://a/2', availability_status: 'expired' } },
    suppressedByUrl: { 'https://a/2': { reason: 'admin_removed', created_at: '2026-07-11T00:00:00Z' } },
  });
  assert.equal(tickets.length, 1);
  assert.equal(tickets[0].dispute, null);
  assert.equal(tickets[0].removed.reason, 'admin_removed');
  assert.equal(tickets[0].status, 'resolved'); // taken down → not an open ticket anymore
});

test('buildListingTickets: an edit dispute with no report is a ticket; open sorts first', () => {
  const tickets = buildListingTickets({
    reports: [{ job_id: 'jResolved', status: 'expired', reported_at: '2026-07-01T00:00:00Z', apply_url: 'https://a/x' }],
    disputes: [{ id: 9, job_id: 'jEdit', company_name: 'Acme', kind: 'edit', detail: 'fix title', status: 'open', created_at: '2026-07-20T00:00:00Z' }],
    jobsById: {},
    suppressedByUrl: { 'https://a/x': { reason: 'admin_removed', created_at: '2026-07-02T00:00:00Z' } },
  });
  assert.equal(tickets.length, 2);
  assert.equal(tickets[0].job_id, 'jEdit');   // open ticket first
  assert.equal(tickets[0].status, 'open');
  assert.equal(tickets[1].status, 'resolved');
});
