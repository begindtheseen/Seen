  ...
  try {
    ...
    return (data.results || []).map(j => {
      const row = {
        ...
        last_seen_at: now,
        last_checked_at: now,
        // Adzuna geocodes every listing — keep the coordinates for real distance/radius math.
        lat: Number.isFinite(Number(j.latitude)) ? Number(j.latitude) : null,
        lng: Number.isFinite(Number(j.longitude)) ? Number(j.longitude) : null,
      };
