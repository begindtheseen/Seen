...
export async function upsertJobs(jobs, supabaseUrl, serviceKey) {
  ...
  // A rejected batch is NOT an empty batch. Returning upserted:0 with no log made a total write
  // failure — a constraint violation, a statement timeout, a schema drift — indistinguishable from
  // "the sources found nothing", which is how a dead ingestion path can look healthy for weeks.
  if (!res.ok) {
    const detail = (await res.text().catch(() => '')).slice(0, 300);
    console.error(`upsertJobs failed: ${res.status} ${detail} (${jobs.length} rows dropped)`);
    return { upserted: 0, rows: [], error: `${res.status} ${detail.slice(0, 160)}` };
  }
