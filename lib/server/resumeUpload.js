...
export async function extractUploadText({ base64, fileName, mimeType }) {
  ...
  // Belt-and-suspenders: the SAME low-word-density signal the optimize/scanner gate applies
  // (api/resume.js). Some spaced-out / glyph-code extractions slip past looksLikeGarbledText but
  // have almost no real words — if we stored them, the résumé would upload "fine" and then fail
  // later with RESUME_CORRUPTED. Rejecting here keeps the two gates consistent so a résumé that
  // saves is always one that can be optimized. (.docx text is clean, so gate only PDF/plain-text.)
  if (isPDF || isText) {
    const alphaWords = (extractedText.match(/[A-Za-z]{3,}/g) || []).length;
    if (extractedText.length > 100 && alphaWords < 20) {
      return {
        ok: false,
        status: 422,
        error: 'We couldn’t read this résumé as text — too few real words came through (often a scanned or phone-exported PDF). Paste your résumé text directly into the box, or re-export the PDF from Word / Google Docs / Pages.',
      };
    }
  }
