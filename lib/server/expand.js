...
export async function getQueryExpansion(qNorm, supabaseUrl, dbHeaders /* , anthropicKey (unused — gateway picks the provider) */) {
  ...
  try {
    const parsed = await chat({
      ...
      timeoutMs: 12000,
      json: true,
      user: `Given a job search query, return its canonical form and related search terms that name the SAME job.

Return ONLY valid JSON: {"canonical":"...","related":["...","...","..."]}

Rules:
- canonical: the single most standard industry job title (lowercase, 1-6 words)
- related: 5-8 terms that name the SAME role a job seeker OR recruiter would post. Each term must be a specific ROLE TITLE or true synonym, NOT a broad category. Include:
  • Common abbreviations AND their expansions (ML↔machine learning engineer, SWE↔software engineer, RN↔registered nurse, NP↔nurse practitioner, CNA↔certified nursing assistant, CDL↔truck driver, HVAC↔hvac technician, PM↔product manager, UX↔user experience designer, QA↔quality assurance, DSP↔delivery driver, etc.)
  • Synonym job titles for the SAME work (software engineer↔software developer↔programmer; budtender↔dispensary associate↔cannabis associate)
  • Industry-specific alternate names recruiters actually post for this exact role
- STRICT relevance — a term is WRONG unless someone doing the query's job would hold that exact title:
  • Do NOT output a generic standalone word ("sales", "manager", "associate", "assistant", "representative", "coordinator", "specialist", "agent", "retail", "customer service") — these match unrelated jobs. Only use such a word as part of a specific role phrase ("dispensary associate", not "associate").
  • Do NOT broaden to a parent/adjacent category or a different seniority of a different job. "budtender" must NOT return "sales", "retail sales", "store manager" — it returns cannabis/dispensary roles only.
