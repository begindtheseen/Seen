...
//
// Every list is capped, so every cap is DECLARED: `totals` carries the uncapped size of each list and
// `truncated` names the ones that were cut. A bounded view that cannot say it is bounded reads as
// "nothing else happened" (the silent-zero class) — a session once concluded no facts were recorded
// today because today's were the ones cut. `changed.*` arrives newest-first from whatsChanged, so a
// cut only ever drops the oldest changes.
export const BRIEFING_CAPS = Object.freeze({ added: 8, invalidated: 8, openThreads: 10, contradictions: 5, lowConfidence: 5 });
export async function briefing({ today = isoToday(), since = null } = {}) {
  const b = buildBriefing(buildIndex(await fetchNotes()), { today, since });
  const lists = {
    added: b.changed.added, invalidated: b.changed.invalidated,
    openThreads: b.openThreads, contradictions: b.contradictions, lowConfidence: b.lowConfidence,
  };
  const cut = (k) => lists[k].slice(0, BRIEFING_CAPS[k]);
  const totals = Object.fromEntries(Object.keys(BRIEFING_CAPS).map((k) => [k, lists[k].length]));
  return {
    today: b.today,
    since: b.since,
    counts: b.counts,
    changed: { added: cut('added'), invalidated: cut('invalidated') },
    openThreads: cut('openThreads'),
    contradictions: cut('contradictions'),
    lowConfidence: cut('lowConfidence'),
    totals,
    truncated: Object.keys(BRIEFING_CAPS).filter((k) => totals[k] > BRIEFING_CAPS[k]),
  };
}
...
export async function putNote(path, content) {
  ...
  const rows = facts.map((f) => ({
    ...
    by: f.by || null, change: f.change || null, supersedes: f.supersedes || null, synced_at: at,
  }));
  // One database transaction owns the canonical note and its exact derived mirror. The RPC takes a
  // transaction-scoped advisory lock per path, so concurrent writers cannot let one request's stale
  // deletion race after another request's winning note/fact update.
  await rest('rpc/brain_replace_note', {
    method: 'POST',
    body: { p_path: notePath, p_content: content, p_updated_at: at, p_facts: rows },
