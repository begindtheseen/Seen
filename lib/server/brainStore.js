...
export async function putNote(path, content) {
  ...
  // Read the old mirror first. A note rewrite can remove or rename facts; merge-upserting the new
  // facts alone leaves those removed rows alive forever, so search/contradiction reads disagree with
  // the canonical markdown. Fetching before the note write also means a failed inventory changes
  // nothing. The convergence order below is deliberately additive-before-subtractive:
  //   note -> current fact upserts -> stale fact deletes.
  // A failure can temporarily leave extras, but can never erase a still-current fact; retrying
  // converges. Stale deletes are individually keyed because fact IDs are free-form strings and must
  // not be interpolated into PostgREST's `in.(...)` grammar.
  const oldRows = await rest(
    `brain_facts?select=id&note=eq.${encodeURIComponent(notePath)}`,
  ) || [];
  ...
  const currentIds = new Set(facts.map((f) => String(f.id)));
  const staleIds = [...new Set(oldRows.map((r) => String(r.id)))]
    .filter((id) => !currentIds.has(id));
  for (const id of staleIds) {
    await rest(
      `brain_facts?note=eq.${encodeURIComponent(notePath)}&id=eq.${encodeURIComponent(id)}`,
