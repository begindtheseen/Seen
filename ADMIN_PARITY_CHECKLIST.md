...
- **Status in Next.js**: ✅ **ported** (recovery branch). `FlagsPanel` component: "+ Seed defaults" header button, per-flag name + description, colored status label (parity colors), 5-option `<select>` with optimistic local override + "Saving…" label (reverts on failure), empty state "Initialize default flags →". POSTs `set_flag {flag_name, status}` / `seed_flags` (seed → refetch). Verifies `d.ok`. NOTE: old SPA also patched in-memory `FeatureFlags._flags` for instant single-page effect; in Next.js, runtime flag consumption re-reads server state on navigation, so no in-memory sync needed in the admin page.
- **Files to change**: `app/admin/page.tsx` ✅ done
