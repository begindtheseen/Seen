...
// Company "Hiring Grade" share card — built on the EXACT same generator as the
// outcome card (components/OutcomeCard.tsx): self-contained 1080×1080 canvas with the
// same background, eyebrow, name, dividers, big colored headline, fact list, footer,
...
async function drawCard(d: CompanyScoreCardData): Promise<HTMLCanvasElement> {
  ...
  const risk = d.risk ?? (score >= 70 ? 'safe' : score >= 40 ? 'warn' : 'danger')
  ...
  const facts: { e: string; t: string }[] = [
    { e: '👻', t: `Ghost Rate: ${Math.round((d.ghost_rate || 0) * 100)}%` },
    { e: '📨', t: `Response Rate: ${Math.round((d.response_rate || 0) * 100)}%` },
    { e: '⏱', t: `Average Wait: ${Math.round(d.avg_wait_days || 0)} Days` },
    { e: '⚠️', t: `Wasted-Time Risk: ${Math.round(d.waste || 0)}%` },
    ...(d.report_count ? [{ e: '📈', t: `Based on ${d.report_count.toLocaleString()} Applicant Reports` }] : []),
  ].slice(0, 6)
  ...
  while (c.measureText(d.company || 'Company').width > W - PAD * 2 && coFs > 32) {
    coFs -= 2; c.font = `800 ${coFs}px "Syne",sans-serif`
  }
  c.fillStyle = '#fff'; c.textAlign = 'left'; c.textBaseline = 'top'
  c.fillText(d.company || 'Company', PAD, coY)

  // Industry (role-equivalent subline)
  const subY = coY + coFs + 10
  c.font = '400 21px "DM Mono",monospace'; c.fillStyle = 'rgba(255,255,255,0.42)'
  let sub = d.industry || `${gradeEmoji} ${risk === 'safe' ? 'Strong process' : risk === 'danger' ? 'High risk' : 'Mixed signals'}`
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(2,4,10,0.97)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1.5rem 1.25rem 2.5rem', overflowY: 'auto' }}>
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        <div style={{ fontFamily: 'var(--mono)', fontSize: '.6rem', color: 'rgba(255,255,255,0.35)', marginBottom: '1.1rem', lineHeight: 1.7 }}>Share this company&apos;s hiring grade — help the next applicant know before they apply.</div>

        {dataUrl ? (
          <img src={dataUrl} alt="Grade card" style={{ width: '100%', borderRadius: 12, boxShadow: `0 24px 80px rgba(0,0,0,0.9), 0 0 60px ${OC}22`, marginBottom: '1rem', display: 'block' }} />
        ) : (
