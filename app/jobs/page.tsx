...
function JobCard({ job, index, onSaveToggle, onOpen, onApply, onCheckCompany }: { job: Job; index: number; onSaveToggle: (id: string) => void; onOpen: (id: string) => void; onApply: (job: Job) => void; onCheckCompany: (company: string) => void }) {
      ...
      <div className="jpage">
        ...
        {filtered.length > 0 ? (
          <div className="jlist" key={saveVersion}>
            {filtered.map((job, i) => (
              <JobCard key={job.id} job={job} index={i} onSaveToggle={handleSaveToggle} onOpen={id => router.push(`/jobs/${encodeURIComponent(id)}`)} onApply={j => { setApplyJob(j); setApplyPlatform('Seen') }} onCheckCompany={co => setCheckCompany(co)} />
            ))}
          </div>
        ) : status === 'idle' ? (
